...
def getInfor():
    cursor = conn.cursor()
    cursor.execute(
        'SELECT masv, fullname, gender, address, birthday, name FROM students, class where students.malop= class.malop')
    a = []
    for i in cursor:
        a.append(list(i))
    conn.commit()
...
def lookInfor(masv):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT masv, fullname, gender, address, birthday, name FROM students, class where students.malop= class.malop and masv = '{0}'".format(
            masv))
    a = []
    for i in cursor:
        a.append(list(i))
...
def insertStudent(masv, name, gender, adress, birthday, malop):
    cursor = conn.cursor()
    query = "insert into Students values('{0}',N'{1}',N'{2}',N'{3}','{4}','{5}')".format(masv, name, gender, adress,
                                                                                         birthday, malop)
...
def editStudent(masv, name, gender, adress, birthday, malop):
    cursor = conn.cursor()
    query = "update Students set fullname=N'{0}', gender=N'{1}', address=N'{2}', birthday='{3}', malop='{4}' where masv='{5}'".format(
        name, gender, adress, birthday, malop, masv)
    cursor.execute(query)
    conn.commit()
...
def editTeachers(magv, name, diachi, sdt):
    cursor = conn.cursor()
    query = "update Teachers set fullname=N'{0}', address=N'{1}', phone='{2}' where magv='{3}'".format(name, diachi,
                                                                                                       sdt, magv)
    cursor.execute(query)
    conn.commit()
